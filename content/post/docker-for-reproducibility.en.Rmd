...
[repository](https://hub.docker.com) of *images* which save users time and effort.

# Building a docker image

Docker works using concepts of ***images*** and ***containers***. I won't go into details about docker
...
syntax so be precise. Either in your command line editor, or other code editor open start editing 
the Dockerfile. 
...
Our analysis-specific image will add our specific needs to the
[rocker/tidyverse](https://www.rocker-project.org/images/) image. Including the version tag
ensures that the image will be built with the version of R and that all installed packages 
will be versioned to work with this R-version. 
...
ADD ./R /home/rstudio_user/R
```

In the next few lines of the Dockerfile, we'll create an analysis output folder and a data 
folder before installing all of the packages we defined in the `packages.R` file. 

```{r}
# add output folder
RUN mkdir /home/rstudio_user/output
# add raw data folder
RUN mkdir /home/rstudio_user/raw
# install packages
...
docker build -t my-analysis-image .
```
...
To check that you image has been built successfully, you can use the command `docker images` 
to output a list of all of your computer's docker images. Confirm that your named image 
`my-analysis-image` is present.
...
The container includes a RStudio server with the username - `rstudio_user` - and password - `test1234` -  
defined in the run command. 

In your internet browser, navigate to [`http://localhost::8787`](http://localhost::8787) where 
a Rstudio login page should allow you to enter the necessary credentials. The RStudio session 
should include a `R` folder containing your analysis scripts. 

Your raw data should be available in the `raw` folder and can be adjusted as required. Raw data stays on
your computer here, as does any outputs from the analysis. 

When you've completed your analysis, log out of the RStudio session and head back to your command line
...
both reproducible and transferable to other computers. The image I've described is quite simple
and could be improved upon both in terms of functionality as well as size. However, the purpose 
of this post was to describe a relatively simple workflow to encourage you to incorporate this final
...
the [Github repo](https://www.github.com/JayAchar/blog-reproduce). 
